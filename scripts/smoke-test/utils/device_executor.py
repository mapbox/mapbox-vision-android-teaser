...
class DeviceExecutor:
    ...
    def __init__(self, device_serial_name, log_adb_commands=False):
        self.command_runner = CommandRunner(log_adb_commands)
    ...
    def install_apk(self, apk_path, grant_all_permissions=False):
        ...
        if grant_all_permissions:
            grant_value = ' -g'
        self.command_runner.execute_command('adb -s %s install%s %s' % (self.device_serial_name, grant_value, apk_path))
    ...
    def tap(self, x, y):
        self.command_runner.execute_command_with_answer(
            'adb -s %s shell input tap %d %d' % (self.device_serial_name, x, y))

    def open_activity(self, package_name, activity_name, params=""):
        self.command_runner. \
            execute_command('adb -s %s shell am start -n %s/%s %s' %
                            (self.device_serial_name, package_name, activity_name, params))

    def get_view_tree(self, view_tree_path, view_tree_file):
        self.folder_helper.create_folder_if_not_exist(view_tree_path)
        self.command_runner. \
            execute_command_with_answer(
    ...
    def clear_device_logs(self):
        self.command_runner.execute_command_with_answer('adb -s %s logcat -c' % self.device_serial_name)
    ...
    def get_device_logs(self, log_path, log_file, level='D'):
        ...
        self.folder_helper.create_folder_if_not_exist(log_path)
        self.command_runner. \
            execute_command_with_answer(
    ...
    def take_screenshot(self, output_path, scree_name):
        self.command_runner. \
            execute_command_with_answer('adb -s %s shell /system/bin/screencap -p /sdcard/smoke_screenshot.png' %
                                        self.device_serial_name)
        self.folder_helper. \
            create_folder_if_not_exist('%s/%s/images/' %
                                       (output_path, self.device_serial_name))
        self.command_runner. \
            execute_command_with_answer('adb -s %s pull /sdcard/smoke_screenshot.png %s/%s/images/%s.png' %
                                        (self.device_serial_name, output_path, self.device_serial_name, scree_name))

    def remove_screenshot_tmp_file(self):
        self.command_runner. \
            execute_command('adb -s %s shell rm /sdcard/smoke_screenshot.png' % self.device_serial_name)
